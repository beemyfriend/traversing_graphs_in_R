...
* A symetric matrix is very different if the output graph is directed or not.
    ...
    + if `mode = "undierected"` then only one element is chosen from either matrix[i][j] or matrix[j][i] 
...
g <- graph_from_adjacency_matrix(matData, mode = 'directed', weighted = 'connections') 
